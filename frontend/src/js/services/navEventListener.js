...
function handleNavAction(
    ...
    switch (action) {
        case "add-todo":
            modalUIController.displayAddTodoModal(todos);
            break;
        case "my-todos":
            updateHeadingAndDisplay("My Todos", (todos) =>
                todoUIController.displayAllTodos(todos)
            );
            break;
        case "today":
            updateHeadingAndDisplay("Today", (todos) =>
                todoUIController.displayTodayTodos(todos)
            );
            break;
        case "upcoming":
            updateHeadingAndDisplay("Upcoming", (todos) =>
                todoUIController.displayUpcomingTodos(todos)
            );
            break;
        case "anytime":
            updateHeadingAndDisplay("Anytime", (todos) =>
                todoUIController.displayAnytimeTodos(todos)
            );
            break;
        case "importance":
            updateHeadingAndDisplay("Importance", (todos) =>
